...
int main(int, char*[])
{
   ...
   depth_first_search(g , visitor(vis)); // lib boostgraphe

   //Création des images du graphe et du graphe dual
   write_graphviz(fout, g, make_label_writer(&names[0])); // lib boostgraphe
   system("dot -Tpng graph.dot > graph.png");
   write_graphviz(fout3, make_reverse_graph(g), make_label_writer(&names[0])); // lib boostgraphe
   system("dot -Tpng dual.dot > dual.png");
   write_graphviz(fout2, g, make_label_writer(&names[0])); // lib boostgraphe
   ...
   reverse(datefinDecroissant.begin(), datefinDecroissant.end()); // lib boostgraphe
   ...
   for (int i = 0; i < datefinDecroissant.size(); i++)
   {
            ...
            if (it != vect.end())
            {
                  ...
                  depth_first_visit(make_reverse_graph(g), i, tempVis, colormap); // lib boostgraphe
