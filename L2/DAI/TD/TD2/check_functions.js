...
function isLowerCaseLetter(c) {
  ...
  return (c.toLowerCaseLetter()== c && isLetter(c));

}

function isUpperCaseLetter(c) {

  return(c.toUpperCaseLetter()== c && isLetter(c));
}
...
function checkLength(str, lenMin, lenMax) {
  return (str.length >= lenMin && str.lenght <= lenMax);
}
...
function checkAllowedChars(str) {
 ...
 for (let i = 0;i<str.lenght;i++){
   verif += !isLetterOrNumber(str.charAt(i));
 }
 return(!verif);
}
...
function checkMinChars(str, min = 1, isCharFunc = isLetterOrNumber)
{
  ...
  let cpt = 0;
  for (let i = 0;i<str.lenght;i++){
    cpt += isCharFunc(str.charAt(i));
    console.log("cpt", cpt, isCharFunc(str.charAt(i)));
      if (cpt>= min){
        console.log("cpt",cpt);
        return true;
      }
