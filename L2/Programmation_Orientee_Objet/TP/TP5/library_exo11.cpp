...
class book_t {
  ...
  public:
    ...
    long get_isbn() const{return this->isbn;}
};
...
class library_t {
  ...
  public:
    ...
    vector<long> get_isbns() const{
      vector<long> isbns;
      for(int i = 0; i < this->nbooks; i++){
        isbns.push_back(books[i].get_isbn());
      }
      return isbns;
    }
};
...
int main(){
  ...
  vector<long> isbn;
  vector<long>::iterator i1; // Déclaration d'un iterateur
  ...
  isbn = Sciences_Library.get_isbns(); //On recupère le vector de retour de l'appel de la méthode

  for(i1 = isbn.begin(); i1!=isbn.end(); ++i1){ //Affichage du vector à  l'aide d'un iterateur
    cout << *i1 << endl;
  }

  sort(isbn.begin(), isbn.end()); //Tri du vector à l'aide de sort

  cout << "Sorted:" << endl;
  for(i1 = isbn.begin(); i1!=isbn.end(); ++i1){
    cout << *i1 << endl;
  }
  return 0;
}
